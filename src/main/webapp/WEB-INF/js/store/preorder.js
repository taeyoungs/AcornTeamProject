$(function(){
	...
	$("#open_order_productions").on('click', function(){
		
		if($(this).attr('data-click-state')==1){
			$(this).attr('data-click-state', 0)
			$(this).text("주문 상품 열기")
			$("#open_order_productions").attr('display','none')
		}else{
			$(this).attr('data-click-state', 1)
			$(this).text("주문 상품 닫기")
			$("#open_order_productions").attr('display','block')
		}
	});

	$("#open_order_productions").click(function(){
		$("#order_productions").toggle();
	});
	...
	$("#delivery_message").on('click',function(){
		$("#delivery_message_presets").css('display','block');
	});
	
	$(".preset").on('click', function(){
		var msg = $(this).text();
		$("#delivery_message").attr('value', msg);
